...
static int replace_s(struct SN_env *z, int c_bra, int c_ket, int s_size, const symbol *s,
		int *adjustment) __attribute__((unused));
...
static int out_grouping_b_U(struct SN_env *z, const unsigned char *s, int min, int max,
		int repeat) {
	do {
		int ch;
		int w = get_b_utf8(z->p, z->c, z->lb, &ch);
...
static int eq_v(struct SN_env *z, const symbol *p) { return eq_s(z, SIZE(p), p); }

static int eq_v_b(struct SN_env *z, const symbol *p) { return eq_s_b(z, SIZE(p), p); }
...
static int slice_del(struct SN_env *z) { return slice_from_s(z, 0, 0); }
...
static void SN_close_env(struct SN_env *z, int S_size) {
	...
	if (S_size) {
		int i;
		for (i = 0; i < S_size; i++) { lose_s(z, z->S[i]); }
...
const sb_symbol *sb_stemmer_stem(struct SN_env *z, const sb_symbol *word, int size) {
	int ret;
...
struct stemmer_modules *sb_stemmer_get(enum SnowballLanguage lang) {
