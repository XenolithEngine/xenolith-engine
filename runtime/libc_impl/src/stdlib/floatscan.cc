...
#if LDBL_MANT_DIG == 53 && LDBL_MAX_EXP == 1'024

#define LD_B1B_DIG 2
#define LD_B1B_MAX 9'007'199, 254'740'991
#define KMAX 128

#elif LDBL_MANT_DIG == 64 && LDBL_MAX_EXP == 16'384

#define LD_B1B_DIG 3
#define LD_B1B_MAX 18, 446'744'073, 709'551'615
#define KMAX 2'048

#elif LDBL_MANT_DIG == 113 && LDBL_MAX_EXP == 16'384

#define LD_B1B_DIG 4
#define LD_B1B_MAX 10'384'593, 717'069'655, 257'060'992, 658'440'191
#define KMAX 2'048
...
static long double decfloat(FILE *f, int c, int bits, int emin, int sign, int pok, int radix) {
	...
	long double frac = 0;
	long double bias = 0;
	static const int p10s[] = {10, 100, 1'000, 10'000, 100'000, 1'000'000, 10'000'000, 100'000'000};
	...
	while (rp < 9 * LD_B1B_DIG || (rp == 9 * LD_B1B_DIG && x[a] < th[0])) {
		...
		for (k = (z - 1 & MASK);; k = (k - 1 & MASK)) {
			...
			if (tmp > 1'000'000'000) {
				carry = tmp / 1'000'000'000;
				x[k] = tmp % 1'000'000'000;
			} else {
				carry = 0;
				x[k] = tmp;
			}
	...
	if ((a + i & MASK) != z) {
		...
		if (t < 500'000'000 && (t || (a + i + 1 & MASK) != z)) {
			frac += 0.25 * sign;
		} else if (t > 500'000'000) {
			frac += 0.75 * sign;
		} else if (t == 500'000'000) {
			if ((a + i + 1 & MASK) == z) {
				frac += 0.5 * sign;
