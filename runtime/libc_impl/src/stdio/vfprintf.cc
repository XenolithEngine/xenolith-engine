...
static int fmt_fp(FILE *f, long double y, int w, int p, int fl, int t, int ps,
	...
	if (e2 < 0) {
		...
		a = r = z = big + sizeof(big) / sizeof(*big) - max_mant_slots - 1;
	}
	...
	while (e2 > 0) {
		uint32_t carry = 0;
		int sh = sprt::min(29, e2);
		for (d = z - 1; d >= a; d--) {
			uint64_t x = ((uint64_t)*d << sh) + carry;
			*d = x % 1'000'000'000;
			carry = x / 1'000'000'000;
		}
		if (carry) {
			*--a = carry;
		}
	...
	while (e2 < 0) {
		uint32_t carry = 0, *b;
		int sh = sprt::min(9, -e2), need = 1 + (p + LDBL_MANT_DIG / 3U + 8) / 9;
		for (d = a; d < z; d++) {
			uint32_t rm = *d & (1 << sh) - 1;
			*d = (*d >> sh) + carry;
			carry = (1'000'000'000 >> sh) * rm;
		}
		if (!*a) {
			a++;
		}
	...
	if (j < 9 * (z - r - 1)) {
		...
		if (x || d + 1 != z) {
			...
			long double small;
			if ((*d / i & 1) || (i == 1'000'000'000 && d > a && (d[-1] & 1))) {
				round += 2;
			}
			if (x < i / 2) {
				small = 0x0.8p0;
	...
	if ((t | 32) == 'g') {
		...
		if (!(fl & ALT_FORM)) {
			...
			if ((t | 32) == 'f') {
				p = sprt::min(ptrdiff_t(p), sprt::max(ptrdiff_t(0), 9 * (z - r - 1) - j));
			} else {
				p = sprt::min(ptrdiff_t(p), sprt::max(ptrdiff_t(0), 9 * (z - r - 1) + e - j));
			}
		}
	}
...
__SPRT_C_FUNC int vsprintf(char *__restrict s, const char *__restrict fmt,
		va_list ap) __SPRT_NOEXCEPT {
...
__SPRT_C_FUNC int snprintf(char *__restrict s, size_t n, const char *__restrict fmt,
		...) __SPRT_NOEXCEPT {
	int ret;
	va_list ap;
	va_start(ap, fmt);
...
__SPRT_C_FUNC int sprintf(char *__restrict s, const char *__restrict fmt, ...) __SPRT_NOEXCEPT {
	int ret;
	va_list ap;
	va_start(ap, fmt);
...
__SPRT_C_FUNC int asprintf(char **s, const char *fmt, ...) __SPRT_NOEXCEPT {
	int ret;
	va_list ap;
	va_start(ap, fmt);
...
__SPRT_C_FUNC int dprintf(int fd, const char *__restrict fmt, ...) __SPRT_NOEXCEPT {
	int ret;
	va_list ap;
	va_start(ap, fmt);
