...
#include "../stdlib/shgetc.h"
#include "../stdlib/intscan.h"
...
static int __vfscanf_l(FILE *__restrict f, const char *__restrict fmt, va_list ap, int radix) {
	...
	for (p = (const unsigned char *)fmt; *p; p++) {
		...
		case '[':
			...
			if (size == SIZE_l) {
				...
				while (scanset[(c = shgetc(f)) + 1]) {
					...
					case __SPRT_SIZE_MAX: goto input_fail;
					case __SPRT_SIZE_MAX - 1: continue;
					}
					if (wcs) {
						wcs[i++] = wc;
					}
		...
		case 'p':
		case 'X':
		case 'x': base = 16; goto int_common;
		case 'o': base = 8; goto int_common;
		case 'd':
		case 'u': base = 10; goto int_common;
		case 'i':
			base = 0;
int_common:
			x = __intscan(f, base, 0, __SPRT_ULLONG_MAX);
			if (!shcnt(f)) {
				goto match_fail;
			}
			if (t == 'p' && dest) {
				*(void **)dest = (void *)(__sprt_uintptr_t)x;
			} else {
		...
		case 'G':
			...
			if (dest) {
				...
				case SIZE_L: *(long double *)dest = y; break;
				}
			}
			break;
		}
