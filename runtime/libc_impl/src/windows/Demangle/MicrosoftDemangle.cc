...
static bool startsWithDigit(sprt::StringView S) { return !S.empty() && sprt::isdigit(S.front()); }
...
static bool consumeFront(sprt::StringView &S, char C) {
	if (!S.starts_with(C)) {
		return false;
	}
	++S;
...
static bool consumeFront(sprt::StringView &S, sprt::StringView C) {
	if (!S.starts_with(C)) {
		return false;
	}
	S += C.size();
...
static bool consumeFront(sprt::StringView &S, sprt::StringView PrefixA, sprt::StringView PrefixB,
		bool A) {
	const sprt::StringView &Prefix = A ? PrefixA : PrefixB;
	return consumeFront(S, Prefix);
}

static bool startsWith(sprt::StringView S, sprt::StringView PrefixA, sprt::StringView PrefixB,
		bool A) {
	const sprt::StringView &Prefix = A ? PrefixA : PrefixB;
	return S.starts_with(Prefix);
}

bool Demangler::isMemberPointer(sprt::StringView MangledName, bool &Error) {
	Error = false;
...
static SpecialIntrinsicKind consumeSpecialIntrinsicKind(sprt::StringView &MangledName) {
...
static bool startsWithLocalScopePattern(sprt::StringView S) {
	if (!consumeFront(S, '?')) {
		return false;
	}

	size_t End = S.find('?');
	if (End == sprt::Max<size_t>) {
		return false;
	}
	sprt::StringView Candidate = S.sub(0, End);
	if (Candidate.empty()) {
		return false;
	}
...
static bool isTagType(sprt::StringView S) {
	switch (S.front()) {
	case 'T': // union
	case 'U': // struct
	case 'V': // class
...
static bool isCustomType(sprt::StringView S) { return S[0] == '?'; }

static bool isPointerType(sprt::StringView S) {
	if (S.starts_with("$$Q")) { // foo &&
		return true;
	}
...
static bool isArrayType(sprt::StringView S) { return S[0] == 'Y'; }

static bool isFunctionType(sprt::StringView S) {
	return S.starts_with("$$A8@@") || S.starts_with("$$A6");
}

static FunctionRefQualifier demangleFunctionRefQualifier(sprt::StringView &MangledName) {
...
static sprt::pair<Qualifiers, PointerAffinity> demanglePointerCVQualifiers(
		sprt::StringView &MangledName) {
	if (consumeFront(MangledName, "$$Q")) {
		return sprt::make_pair(Q_None, PointerAffinity::RValueReference);
	}

	const char F = MangledName.front();
	MangledName += 1;
	switch (F) {
	case 'A': return sprt::make_pair(Q_None, PointerAffinity::Reference);
	case 'P': return sprt::make_pair(Q_None, PointerAffinity::Pointer);
	case 'Q': return sprt::make_pair(Q_Const, PointerAffinity::Pointer);
	case 'R': return sprt::make_pair(Q_Volatile, PointerAffinity::Pointer);
	case 'S': return sprt::make_pair(Qualifiers(Q_Const | Q_Volatile), PointerAffinity::Pointer);
	}
...
sprt::StringView Demangler::copyString(sprt::StringView Borrowed) {
	...
	// buffer.
	if (Borrowed.size()) {
		sprt::memcpy(Stable, Borrowed.data(), Borrowed.size());
	}

	return {Stable, Borrowed.size()};
}

SpecialTableSymbolNode *Demangler::demangleSpecialTableSymbolNode(sprt::StringView &MangledName,
...
LocalStaticGuardVariableNode *Demangler::demangleLocalStaticGuard(sprt::StringView &MangledName,
		bool IsThread) {
...
static QualifiedNameNode *synthesizeQualifiedName(ArenaAllocator &Arena, sprt::StringView Name) {
	NamedIdentifierNode *Id = synthesizeNamedIdentifier(Arena, Name);
	return synthesizeQualifiedName(Arena, Id);
}

static VariableSymbolNode *synthesizeVariable(ArenaAllocator &Arena, TypeNode *Type,
		sprt::StringView VariableName) {
...
VariableSymbolNode *Demangler::demangleUntypedVariable(ArenaAllocator &Arena,
...
VariableSymbolNode *Demangler::demangleRttiBaseClassDescriptorNode(ArenaAllocator &Arena,
		sprt::StringView &MangledName) {
...
FunctionSymbolNode *Demangler::demangleInitFiniStub(sprt::StringView &MangledName,
		bool IsDestructor) {
...
SymbolNode *Demangler::demangleSpecialIntrinsic(sprt::StringView &MangledName) {
	SpecialIntrinsicKind SIK = consumeSpecialIntrinsicKind(MangledName);

	switch (SIK) {
	case SpecialIntrinsicKind::None: return nullptr;
...
IdentifierNode *Demangler::demangleFunctionIdentifierCode(sprt::StringView &MangledName) {
	assert(MangledName.starts_with('?'));
	MangledName += 1;
	if (MangledName.empty()) {
		Error = true;
		return nullptr;
...
StructorIdentifierNode *Demangler::demangleStructorIdentifier(sprt::StringView &MangledName,
		bool IsDestructor) {
...
ConversionOperatorIdentifierNode *Demangler::demangleConversionOperatorIdentifier(
		sprt::StringView &MangledName) {
	ConversionOperatorIdentifierNode *N = Arena.alloc<ConversionOperatorIdentifierNode>();
	return N;
}

LiteralOperatorIdentifierNode *Demangler::demangleLiteralOperatorIdentifier(
		sprt::StringView &MangledName) {
...
IntrinsicFunctionKind Demangler::translateIntrinsicFunctionCode(char CH,
...
SymbolNode *Demangler::demangleEncodedSymbol(sprt::StringView &MangledName,
...
SymbolNode *Demangler::demangleDeclarator(sprt::StringView &MangledName) {
...
SymbolNode *Demangler::demangleMD5Name(sprt::StringView &MangledName) {
	assert(MangledName.starts_with("??@"));
	...
	sprt::StringView MD5(Start, Count);
...
SymbolNode *Demangler::demangleTypeinfoName(sprt::StringView &MangledName) {
	assert(MangledName.starts_with('.'));
...
// Parser entry point.
SymbolNode *Demangler::parse(sprt::StringView &MangledName) {
...
TagTypeNode *Demangler::parseTagUniqueName(sprt::StringView &MangledName) {
...
VariableSymbolNode *Demangler::demangleVariableEncoding(sprt::StringView &MangledName,
		StorageClass SC) {
	VariableSymbolNode *VSN = Arena.alloc<VariableSymbolNode>();
	...
	default: VSN->Type->Quals = demangleQualifiers(MangledName).first; break;
	}
...
sprt::pair<uint64_t, bool> Demangler::demangleNumber(sprt::StringView &MangledName) {
...
uint64_t Demangler::demangleUnsigned(sprt::StringView &MangledName) {
	bool IsNegative = false;
	uint64_t Number = 0;
	auto ret = demangleNumber(MangledName);
	Number = ret.first;
	IsNegative = ret.second;
	if (IsNegative) {
		Error = true;
	}
	return Number;
...
int64_t Demangler::demangleSigned(sprt::StringView &MangledName) {
	bool IsNegative = false;
	uint64_t Number = 0;
	auto ret = demangleNumber(MangledName);
	Number = ret.first;
...
// Memorize it.
void Demangler::memorizeString(sprt::StringView S) {
...
NamedIdentifierNode *Demangler::demangleBackRefName(sprt::StringView &MangledName) {
...
IdentifierNode *Demangler::demangleTemplateInstantiationName(sprt::StringView &MangledName,
	...
	sprt::swap(OuterContext, Backrefs);
...
NamedIdentifierNode *Demangler::demangleSimpleName(sprt::StringView &MangledName, bool Memorize) {
	sprt::StringView S = demangleSimpleString(MangledName, Memorize);
	if (Error) {
		return nullptr;
	}
...
uint8_t Demangler::demangleCharLiteral(sprt::StringView &MangledName) {
	...
	if (consumeFront(MangledName, '$')) {
		...
		sprt::StringView Nibbles = MangledName.sub(0, 2);
	...
	if (MangledName[0] >= 'a' && MangledName[0] <= 'z') {
		char Lookup[26] = {'\xE1', '\xE2', '\xE3', '\xE4', '\xE5', '\xE6', '\xE7', '\xE8', '\xE9',
	...
	if (MangledName[0] >= 'A' && MangledName[0] <= 'Z') {
		char Lookup[26] = {'\xC1', '\xC2', '\xC3', '\xC4', '\xC5', '\xC6', '\xC7', '\xC8', '\xC9',
...
wchar_t Demangler::demangleWcharLiteral(sprt::StringView &MangledName) {
	uint8_t C1, C2;
...
static void outputHex(OutputBuffer &OB, unsigned C) {
	...
	assert(Pos >= 0);
	TempBuffer[Pos--] = '\\';
	OB << sprt::StringView(&TempBuffer[Pos + 1]);
}
...
static unsigned decodeMultiByteChar(const uint8_t *StringBytes, unsigned CharIndex,
		unsigned CharBytes) {
...
FunctionSymbolNode *Demangler::demangleVcallThunkNode(sprt::StringView &MangledName) {
...
EncodedStringLiteralNode *Demangler::demangleStringLiteral(sprt::StringView &MangledName) {
	...
	if (CrcEndPos == sprt::Max<size_t>) {
		goto StringLiteralError;
	}
	CRC = MangledName.sub(0, CrcEndPos);
	MangledName += CrcEndPos + 1;
	if (MangledName.empty()) {
		goto StringLiteralError;
	}
...
sprt::StringView Demangler::demangleSimpleString(sprt::StringView &MangledName, bool Memorize) {
	sprt::StringView S;
	for (size_t i = 0; i < MangledName.size(); ++i) {
		if (MangledName[i] != '@') {
			continue;
...
NamedIdentifierNode *Demangler::demangleAnonymousNamespaceName(sprt::StringView &MangledName) {
	assert(MangledName.starts_with("?A"));
	consumeFront(MangledName, "?A");

	NamedIdentifierNode *Node = Arena.alloc<NamedIdentifierNode>();
	Node->Name = "`anonymous namespace'";
	size_t EndPos = MangledName.find('@');
	if (EndPos == sprt::Max<size_t>) {
		Error = true;
		return nullptr;
	}
	sprt::StringView NamespaceKey = MangledName.sub(0, EndPos);
	memorizeString(NamespaceKey);
	MangledName = MangledName.sub(EndPos + 1);
	return Node;
}

NamedIdentifierNode *Demangler::demangleLocallyScopedNamePiece(sprt::StringView &MangledName) {
...
QualifiedNameNode *Demangler::demangleFullyQualifiedTypeName(sprt::StringView &MangledName) {
...
QualifiedNameNode *Demangler::demangleFullyQualifiedSymbolName(sprt::StringView &MangledName) {
...
IdentifierNode *Demangler::demangleUnqualifiedTypeName(sprt::StringView &MangledName,
		bool Memorize) {
...
IdentifierNode *Demangler::demangleUnqualifiedSymbolName(sprt::StringView &MangledName,
		NameBackrefBehavior NBB) {
	if (startsWithDigit(MangledName)) {
		return demangleBackRefName(MangledName);
	}
...
IdentifierNode *Demangler::demangleNameScopePiece(sprt::StringView &MangledName) {
	if (startsWithDigit(MangledName)) {
		return demangleBackRefName(MangledName);
	}
...
QualifiedNameNode *Demangler::demangleNameScopeChain(sprt::StringView &MangledName,
...
FuncClass Demangler::demangleFunctionClass(sprt::StringView &MangledName) {
	const char F = MangledName.front();
	MangledName += 1;
	switch (F) {
	...
	case 'D': return FuncClass(FC_Private | FC_Static | FC_Far);
	case 'E': return FuncClass(FC_Private | FC_Virtual);
	case 'F': return FuncClass(FC_Private | FC_Virtual | FC_Far);
...
CallingConv Demangler::demangleCallingConvention(sprt::StringView &MangledName) {
	if (MangledName.empty()) {
		Error = true;
	...
	case 'S': return CallingConv::Swift;
	case 'W': return CallingConv::SwiftAsync;
	}

	return CallingConv::None;
}

StorageClass Demangler::demangleVariableStorageClass(sprt::StringView &MangledName) {
...
sprt::pair<Qualifiers, bool> Demangler::demangleQualifiers(sprt::StringView &MangledName) {
	if (MangledName.empty()) {
		Error = true;
		return sprt::make_pair(Q_None, false);
	}
	...
	switch (F) {
	// Member qualifiers
	case 'Q': return sprt::make_pair(Q_None, true);
	case 'R': return sprt::make_pair(Q_Const, true);
	case 'S': return sprt::make_pair(Q_Volatile, true);
	case 'T': return sprt::make_pair(Qualifiers(Q_Const | Q_Volatile), true);
	// Non-Member qualifiers
	case 'A': return sprt::make_pair(Q_None, false);
	case 'B': return sprt::make_pair(Q_Const, false);
	case 'C': return sprt::make_pair(Q_Volatile, false);
	case 'D': return sprt::make_pair(Qualifiers(Q_Const | Q_Volatile), false);
	}
	Error = true;
	return sprt::make_pair(Q_None, false);
}
...
TypeNode *Demangler::demangleType(sprt::StringView &MangledName, QualifierMangleMode QMM) {
	...
	if (isTagType(MangledName)) {
		...
		if (consumeFront(MangledName, "$$A8@@")) {
			...
			assert(MangledName.starts_with("$$A6"));
...
bool Demangler::demangleThrowSpecification(sprt::StringView &MangledName) {
...
FunctionSignatureNode *Demangler::demangleFunctionType(sprt::StringView &MangledName,
		bool HasThisQuals) {
...
FunctionSymbolNode *Demangler::demangleFunctionEncoding(sprt::StringView &MangledName) {
...
CustomTypeNode *Demangler::demangleCustomType(sprt::StringView &MangledName) {
	assert(MangledName.starts_with('?'));
	MangledName += 1;
...
PrimitiveTypeNode *Demangler::demanglePrimitiveType(sprt::StringView &MangledName) {
	...
	case '_': {
		...
		case 'Q': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Char8);
		...
		case 'P': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::Auto);
		case 'T': return Arena.alloc<PrimitiveTypeNode>(PrimitiveKind::DecltypeAuto);
		}
		break;
	}
	}
...
TagTypeNode *Demangler::demangleClassType(sprt::StringView &MangledName) {
...
PointerTypeNode *Demangler::demanglePointerType(sprt::StringView &MangledName) {
	PointerTypeNode *Pointer = Arena.alloc<PointerTypeNode>();

	auto ret = demanglePointerCVQualifiers(MangledName);
	Pointer->Quals = ret.first;
	Pointer->Affinity = ret.second;
...
PointerTypeNode *Demangler::demangleMemberPointerType(sprt::StringView &MangledName) {
	PointerTypeNode *Pointer = Arena.alloc<PointerTypeNode>();
...
Qualifiers Demangler::demanglePointerExtQualifiers(sprt::StringView &MangledName) {
	Qualifiers Quals = Q_None;
...
sprt::optional<PointerAuthQualifierNode::ArgArray> Demangler::demanglePointerAuthQualifier(
		sprt::StringView &MangledName) {
	...
	return sprt::optional<PointerAuthQualifierNode::ArgArray>(Array);
}

PointerAuthQualifierNode *Demangler::createPointerAuthQualifier(sprt::StringView &MangledName) {
	constexpr unsigned NumArgs = PointerAuthQualifierNode::NumArgs;
	sprt::optional<PointerAuthQualifierNode::ArgArray> Vals =
			demanglePointerAuthQualifier(MangledName);
	...
	PointerAuthQualifierNode *PtrAuthQual = Arena.alloc<PointerAuthQualifierNode>();
...
ArrayTypeNode *Demangler::demangleArrayType(sprt::StringView &MangledName) {
...
NodeArrayNode *Demangler::demangleFunctionParameterList(sprt::StringView &MangledName,
		bool &IsVariadic) {
	...
	size_t Count = 0;
	while (!Error && !MangledName.starts_with('@') && !MangledName.starts_with('Z')) {
		++Count;
...
NodeArrayNode *Demangler::demangleTemplateParameterList(sprt::StringView &MangledName) {
	NodeList *Head = nullptr;
	NodeList **Current = &Head;
	size_t Count = 0;

	while (!MangledName.starts_with('@')) {
		...
		*Current = Arena.alloc<NodeList>();
		...
		if (consumeFront(MangledName, "$$Y")) {
			...
			TPRN->Symbol = S;
		} else if (MangledName.starts_with("$E?")) {
	...
	assert(MangledName.starts_with('@')); // The above loop exits only on '@'.
	consumeFront(MangledName, '@');
	return nodeListToNodeArray(Arena, Head, Count);
}

void Demangler::dumpBackReferences() {
	__sprt_printf("%d function parameter backreferences\n", (int)Backrefs.FunctionParamCount);
	...
	__sprt_printf("%d name backreferences\n", (int)Backrefs.NamesCount);
	for (size_t I = 0; I < Backrefs.NamesCount; ++I) {
...
sprt::optional<size_t> llvm::getArm64ECInsertionPointInMangledName(sprt::StringView MangledName) {
	sprt::StringView ProcessedName{MangledName};
	...
	return sprt::optional<size_t>(MangledName.size() - ProcessedName.size());
}

char *llvm::microsoftDemangle(sprt::StringView MangledName, size_t *NMangled, int *Status,
		MSDemangleFlags Flags) {
	Demangler D;

	sprt::StringView Name{MangledName};
