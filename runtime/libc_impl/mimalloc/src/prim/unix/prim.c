...
#include <TargetConditionals.h>
		...
		|| TARGET_OS_OSX // see issue #879, used to be (!TARGET_IOS_IPHONE && !TARGET_IOS_SIMULATOR)
...
static inline int mi_prim_close(int fd) { return syscall(SYS_close, fd); }
...
static inline int mi_prim_close(int fd) { return close(fd); }
static inline int mi_prim_access(const char *fpath, int mode) { return access(fpath, mode); }
...
#elif defined( \
		_SC_PHYS_PAGES) // do not use by default as it might cause allocation (by using `fopen` to parse /proc/meminfo) (issue #1100)
	const long pphys = sysconf(_SC_PHYS_PAGES);
	const size_t psize_in_kib = page_size / MI_KiB;
	if (psize_in_kib > 0 && pphys > 0 && (unsigned long)pphys <= SIZE_MAX
			&& (size_t)pphys <= (SIZE_MAX / psize_in_kib)) {
...
static void *unix_mmap_prim_aligned(void *addr, size_t size, size_t try_alignment,
		int protect_flags, int flags, int fd) {
	...
	if (addr == NULL && try_alignment > 1 && (try_alignment % _mi_os_page_size()) == 0) {
		...
		if (((size_t)1 << n) == try_alignment && n >= 12
			...
			if (p == MAP_FAILED || !_mi_is_aligned(p, try_alignment)) {
				int err = errno;
				_mi_trace_message(
						"unable to directly request aligned OS memory (error: %d (0x%x), size: "
	...
	if (addr == NULL && try_alignment > 1 && (try_alignment % _mi_os_page_size()) == 0) {
		p = unix_mmap_prim((void *)try_alignment, size, protect_flags, flags | MAP_ALIGN,
				fd); // addr parameter is the required alignment
		if (p != MAP_FAILED) {
			return p;
		}
		// fall back to regular mmap
...
#endif
				_mi_trace_message(
						"unable to directly request hinted aligned OS memory (error: %d (0x%x), "
						"size: 0x%zx bytes, alignment: 0x%zx, hint address: %p)\n",
...
static void *unix_mmap(void *addr, size_t size, size_t try_alignment, int protect_flags,
		bool large_only, bool allow_large, bool *is_large) {
...
#define MAP_NORESERVE  0
	...
	if (allow_large
			&& (large_only
					|| (_mi_os_canuse_large_page(size, try_alignment)
			...
			if (large_only || lflags != flags) {
				...
				*is_large = true;
				...
				if (p == NULL && (lflags & MAP_HUGE_1GB) == MAP_HUGE_1GB) {
					...
					if (large_only) {
						_mi_warning_message(
								"unable to allocate huge (1GiB) page, trying large (2MiB) pages "
								"instead (errno: %i)\n",
								errno);
					}
					lflags = ((lflags & ~MAP_HUGE_1GB) | MAP_HUGE_2MB);
					p = unix_mmap_prim_aligned(addr, size, try_alignment, protect_flags, lflags,
							lfd);
				}
#endif
				if (large_only) {
	...
	if (p == NULL) {
		*is_large = false;
		...
		if (p != NULL && allow_large && mi_option_is_enabled(mi_option_allow_thp)
				&& _mi_os_canuse_large_page(size, try_alignment)) {
...
static long mi_prim_mbind(void *start, unsigned long len, unsigned long mode,
...
void _mi_prim_process_info(mi_process_info_t *pinfo) {
	...
	while (get_next_area_info(tid.team, &c, &mem) == B_OK) { pinfo->peak_rss += mem.ram_size; }
	pinfo->page_faults = 0;
#elif defined(__APPLE__)
...
void _mi_prim_out_stderr(const char *msg) { fputs(msg, stderr); }
...
#else
extern char **environ;
static char **mi_get_environ(void) { return environ; }
#endif
...
bool _mi_prim_random_buf(void *buf, size_t buf_len) {
	...
	return true;
		...
		|| defined(__HAIKU__) // also for old apple versions < 10.7 (issue #829)
...
bool _mi_prim_random_buf(void *buf, size_t buf_len) { return false; }
...
#if defined(__EMBOX_USER__)
static pthread_once_t mi_heap_default_key_once = PTHREAD_ONCE_INIT;

static void mi_heap_default_key_create(void) {
	pthread_key_create(&_mi_heap_default_key, &mi_pthread_done);
}

void _mi_prim_thread_init_auto_done(void) {
	// nothing: see mi_heap_default_key_once
}
#else
void _mi_prim_thread_init_auto_done(void) {
...
#if defined(__EMBOX_USER__)
	if (heap == NULL || heap == &_mi_heap_main) {
		return; // the main thread: see above
	}
	pthread_once(&mi_heap_default_key_once, &mi_heap_default_key_create);
#endif
...
void _mi_prim_thread_associate_default_heap(mi_heap_t *heap) { MI_UNUSED(heap); }
