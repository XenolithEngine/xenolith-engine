...
#ifndef RUNTIME_INCLUDE_SPRT_RUNTIME_GEOM_QUATERNION_H_
#define RUNTIME_INCLUDE_SPRT_RUNTIME_GEOM_QUATERNION_H_
...
public:
	...
	constexpr Quaternion(const Vec3 &eulerAngles) {
		...
		float coshalfRadx = sprt::cos(halfRadx), sinhalfRadx = sprt::sin(halfRadx),
			  ...
			  coshalfRadz = sprt::cos(halfRadz), sinhalfRadz = sprt::sin(halfRadz);
	...
	constexpr Quaternion(const Vec3 &axis, float angle) {
		...
		float sinHalfAngle = sprt::sin(halfAngle);
...
#endif /* RUNTIME_INCLUDE_SPRT_RUNTIME_GEOM_QUATERNION_H_ */
