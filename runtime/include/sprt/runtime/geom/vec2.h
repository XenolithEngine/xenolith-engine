...
#ifndef RUNTIME_INCLUDE_SPRT_RUNTIME_GEOM_VEC2_H_
#define RUNTIME_INCLUDE_SPRT_RUNTIME_GEOM_VEC2_H_

#include <sprt/runtime/stringview.h>
#include <sprt/runtime/geom/common.h>
...
public:
	...
	static constexpr sprt::bitset<2> bitop(const Vec2 &v, const Functor &f) {
		sprt::bitset<2> ret;
		ret.set(0, f(v.x));
		ret.set(1, f(v.y));
		return ret;
	...
	template <typename Functor>
	static constexpr sprt::bitset<2> bitop(const Vec2 &v1, const Vec2 &v2, const Functor &f) {
		sprt::bitset<2> ret;
	...
	: x(buf.size() > 0 ? buf[0] : NaN<float>), y(buf.size() > 1 ? buf[1] : NaN<float>) { }
	...
	constexpr float distance(const Vec2 &other) const { return sprt::sqrt(distanceSquared(other)); }

	constexpr float length() const { return sprt::sqrt(lengthSquared()); }
	...
	constexpr float getAngle() const { return sprt::atan2(y, x); }
...
inline bool Vec2::getSegmentIntersectPoint(const Vec2 &A, const Vec2 &B, const Vec2 &C,
	...
	static_assert(sprt::is_invocable_v<Callback, const Vec2 &, float, float>,
			"Invalid callback type");
	float S, T;

	const auto minXAB = sprt::min(A.x, B.x);
	const auto minYAB = sprt::min(A.y, B.y);

	const auto minXCD = sprt::min(C.x, D.x);
	const auto minYCD = sprt::min(C.y, D.y);

	const auto maxXAB = sprt::max(A.x, B.x);
	const auto maxYAB = sprt::max(A.y, B.y);
...
#endif /* RUNTIME_INCLUDE_SPRT_RUNTIME_GEOM_VEC2_H_ */
