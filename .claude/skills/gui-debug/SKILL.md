...
A DEBUG build of any Xenolith GUI app exposes two process-wide debug services over
UNIX sockets, consumed by two MCP tools:

1. **`inspect_scene`** — the live scene graph as an indented text tree (one node
   per line), served at `/tmp/xenolith-inspector.sock`.
2. **`get_logs`** — the application log ring buffer (last ~4096 entries, formatted
   `[LEVEL][tag] message`), served at `/tmp/xenolith-logs.sock`.

Both are wired in `xenolith/application/nodes/XLSceneInspector.cc` (scene snapshot
refreshed ~5×/s; a `stappler::log::CustomLog` sink mirrors every log call into the
ring buffer). Release builds ship neither — zero overhead, no sockets.
...
- Engine side: `XLSceneInspector.cc` (sockets + log sink); scene graph nodes under
