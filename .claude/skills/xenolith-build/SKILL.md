...
description: >-
  ...
  Windows, Android-NDK, macOS). Prefer xenolith-cli; use raw make only
  when the CLI is not installed. Use when asked to build, compile-check, or
  verify a change in this monorepo — including "does this still compile", "build
  tests/window", "check the Windows build", or after editing runtime / stappler /
  xenolith sources. Picks the right test project and target for the code that
  changed, and reports exit status as the pass/fail signal.
---

# Building a Xenolith / Stappler project

**Drive builds through `xenolith-cli` whenever it is on `PATH`.** The
engine's make system (`make/universal.mk`) still does the compile, but the CLI
is the supported entry point — it wires SDK toolchains / `STAPPLER_ROOT` and
invokes make for you. Raw `make` / `gmake` / `xlmake` is **fallback only** when
`command -v xenolith-cli` fails. Never hand-roll a `clang`/`cmake`
command line. Authoritative reference: `AGENTS.md` at the repo root.

## Golden rules

- **CLI first, make second.** At the start of a build session:
  ```sh
  command -v xenolith-cli
  ```
  If found → every build goes through the CLI. If missing → use absolute
  `make -C <abs-path>` as documented below. Do **not** skip the CLI just because
  a Makefile or `stappler-build/` tree exists.
...
- **Every build is a cross-compile**, selected by `--target <triple>` (CLI) or
  `STAPPLER_TARGET=<triple>` (make). With no target the host triple is
  auto-detected. Pass the triple explicitly for reproducibility.
- **Use absolute paths.** Pass absolute project paths (and `--engine`) — the
  shell cwd drifts between calls; never rely on `cd`.
...
### Preferred: `xenolith-cli`

Inside this monorepo always pass `--engine` (or set `$XENOLITH_ENGINE`) to the
absolute engine root so the live checkout is used instead of a baked snapshot:

```sh
ENGINE=<abs-path-to-xenolith-engine>   # this repo root
PROJ=<abs-proj-path>

xenolith-cli build "$PROJ" --engine "$ENGINE" \
    [--target <triple>] [--release] [--run]
```

Outside the monorepo (scaffolded app + installed SDK) omit `--engine`.
...
### Fallback: raw `make` (only if CLI is missing)
...
toolchain problem. On macOS hosts without GNU make 4.1+, use `gmake`.

### Android quirks (these are real and recurring)

Prefer the CLI with a single job (nested `ndk-build` breaks under a parent
jobserver). Make fallback if the CLI is missing:
...
- **Never pass a high `-j`/`-jN` into Android** — nested `ndk-build` dies on the
  make-4.4 fifo jobserver. Prefer `-j1` on the CLI; with raw make, clear
  `MAKEFLAGS` and let ndk-build parallelize itself.
...
- **GUI apps** (`tests/window` → `testapp`) **build** headless but need a display
  ...
  verification signal (`testapp --help` works without a GUI). Or use
  `xenolith-cli … build … --run` when a display is available.
...
(`LOCAL_OUTDIR` defaults to `./stappler-build`). The presence of a
`stappler-build/` directory is **not** an instruction to call `make` — still use
...
file wins over both locations). Run with `verbose=1` (make fallback) to print
which file was picked. The CLI links installed toolchains into the engine tree
...
xenolith_renderer_basic2d xenolith_backend_vk`. Scaffold new apps with
`xenolith-cli new <name>` when the CLI is available.
