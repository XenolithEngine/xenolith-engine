...
### <MODULE>_PRIVATE_FLAGS_FILTER

Набор флагов, которые нужно исключить из списка для этого модуля

### <MODULE>_PRIVATE_STANDALONE

Если переменная определена для модуля, к нему применяются только общие флаги сборки, но не флаги, установленные другими модулями. Модули сами по себе всё также могут добавлять флаги и пути поиска для других модулей.
