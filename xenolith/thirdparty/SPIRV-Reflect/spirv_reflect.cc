...
#define IsNotNull(ptr) (ptr != nullptr)
...
static SpvReflectResult IntersectSortedAccessedVariable(const SpvReflectPrvAccessedVariable *p_arr0,
		size_t arr0_size, const uint32_t *p_arr1, size_t arr1_size, uint32_t **pp_res,
		size_t *res_size) {
	*pp_res = nullptr;
	*res_size = 0;
...
static SpvReflectResult ReadU32(SpvReflectPrvParser *p_parser, uint32_t word_offset,
		uint32_t *p_value) {
...
static SpvReflectResult ReadStr(SpvReflectPrvParser *p_parser, uint32_t word_offset,
		uint32_t word_index, uint32_t word_count, uint32_t *p_buf_size, char *p_buf) {
...
static SpvReflectDecorationFlags ApplyDecorations(
		const SpvReflectPrvDecorations *p_decoration_fields) {
...
static bool IsSpecConstant(const SpvReflectPrvNode *p_node) {
	return (p_node->op == SpvOpSpecConstant || p_node->op == SpvOpSpecConstantOp
			|| p_node->op == SpvOpSpecConstantTrue || p_node->op == SpvOpSpecConstantFalse);
}

static SpvReflectPrvNode *FindNode(SpvReflectPrvParser *p_parser, uint32_t result_id) {
	SpvReflectPrvNode *p_node = nullptr;
...
static SpvReflectTypeDescription *FindType(SpvReflectShaderModule *p_module, uint32_t type_id) {
	SpvReflectTypeDescription *p_type = nullptr;
...
static uint32_t FindAccessChainBaseVariable(SpvReflectPrvParser *p_parser,
...
static SpvReflectResult ParseNodes(SpvReflectPrvParser *p_parser) {
	...
	p_parser->source_embedded = nullptr;
	...
	if (p_parser->access_chain_count > 0) {
		p_parser->access_chains = (SpvReflectPrvAccessChain *)calloc(p_parser->access_chain_count,
				sizeof(*(p_parser->access_chains)));
	...
	while (spirv_word_index < p_parser->spirv_word_count) {
		...
		case SpvOpInBoundsAccessChain: {
			...
			if (p_access_chain->index_count > 0) {
				p_access_chain->indexes = (uint32_t *)calloc(p_access_chain->index_count,
						sizeof(*(p_access_chain->indexes)));
				if (IsNull(p_access_chain->indexes)) {
					return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
				}
				// Parse any index values for access chain
				for (uint32_t index_index = 0; index_index < p_access_chain->index_count;
						++index_index) {
					// Read index id
					uint32_t index_id = 0;
					CHECKED_READU32(p_parser,
							p_node->word_offset + SPIRV_ACCESS_CHAIN_INDEX_OFFSET + index_index,
							index_id);
					// Find OpConstant node that contains index value
					SpvReflectPrvNode *p_index_value_node = FindNode(p_parser, index_id);
					if ((p_index_value_node != nullptr)
...
static SpvReflectResult ParseStrings(SpvReflectPrvParser *p_parser) {
	...
	if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
		...
		p_parser->strings =
				(SpvReflectPrvString *)calloc(p_parser->string_count, sizeof(*(p_parser->strings)));
...
static SpvReflectResult ParseFunction(SpvReflectPrvParser *p_parser, SpvReflectPrvNode *p_func_node,
		SpvReflectPrvFunction *p_func, size_t first_label_index) {
	...
	for (size_t i = first_label_index; i < p_parser->node_count; ++i) {
		...
		case SpvOpFunctionParameter: {
			CHECKED_READU32(p_parser, p_node->word_offset + 2,
					p_func->parameters[p_func->parameter_count]);
			(++p_func->parameter_count);
		} break;
		case SpvOpFunctionCall: {
			...
			for (uint32_t j = 0, parameter_count = p_node->word_count - 4; j < parameter_count;
					j++) {
		...
		case SpvOpStore: {
			const uint32_t result_index = p_node->word_offset + 2;
			CHECKED_READU32(p_parser, result_index,
...
static SpvReflectResult ParseFunctions(SpvReflectPrvParser *p_parser) {
	...
	if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
		...
		qsort(p_parser->functions, p_parser->function_count, sizeof(*(p_parser->functions)),
...
static SpvReflectResult ParseMemberCounts(SpvReflectPrvParser *p_parser) {
	...
	if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
		...
		for (uint32_t i = 0; i < p_parser->node_count; ++i) {
			...
			p_node->member_names =
					(const char **)calloc(p_node->member_count, sizeof(*(p_node->member_names)));
			if (IsNull(p_node->member_names)) {
				return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
			}

			p_node->member_decorations = (SpvReflectPrvDecorations *)calloc(p_node->member_count,
					sizeof(*(p_node->member_decorations)));
...
static SpvReflectResult ParseDecorations(SpvReflectPrvParser *p_parser) {
	...
	for (uint32_t i = 0; i < p_parser->node_count; ++i) {
		...
		if ((p_node->op != SpvOpDecorate) && (p_node->op != SpvOpMemberDecorate)
				&& (p_node->op != SpvOpDecorateId) && (p_node->op != SpvOpDecorateString)
		...
		if (p_node->op == SpvOpDecorateString && decoration == SpvDecorationUserTypeGOOGLE) {
			...
			SpvReflectResult result = ReadStr(p_parser, p_node->word_offset + 3, 0,
					p_node->word_count, &terminator, nullptr);
			if (result != SPV_REFLECT_RESULT_SUCCESS) {
				return result;
			}
...
static SpvReflectResult EnumerateAllUniforms(SpvReflectShaderModule *p_module,
		size_t *p_uniform_count, uint32_t **pp_uniforms) {
...
static SpvReflectResult ParseType(SpvReflectPrvParser *p_parser, SpvReflectPrvNode *p_node,
	...
	if (result == SPV_REFLECT_RESULT_SUCCESS) {
		...
		switch (p_node->op) {
		default: break;
		case SpvOpTypeVoid: p_type->type_flags |= SPV_REFLECT_TYPE_FLAG_VOID; break;

		case SpvOpTypeBool: p_type->type_flags |= SPV_REFLECT_TYPE_FLAG_BOOL; break;
		...
		case SpvOpTypeVector: {
			...
			if (IsNotNull(p_next_node)) {
				result = ParseType(p_parser, p_next_node, nullptr, p_module, p_type);
			} else {
				...
				if (IsNotNull(p_length_node)) {
					...
					if (result == SPV_REFLECT_RESULT_SUCCESS) {
						...
						p_type->traits.array.spec_constant_op_ids[dim_index] =
								IsSpecConstant(p_length_node) ? p_length_node->decorations.spec_id
		...
		case SpvOpTypeStruct: {
			...
			for (; word_index < p_node->word_count; ++word_index, ++member_index) {
				...
				result = ParseType(p_parser, p_member_node, p_member_decorations, p_module,
						p_member_type);
				if (result != SPV_REFLECT_RESULT_SUCCESS) {
					break;
				}
		...
		case SpvOpTypePointer: {
			...
			IF_READU32_CAST(result, p_parser, p_node->word_offset + 2, SpvStorageClass,
					p_type->storage_class);
...
static SpvReflectResult ParseTypes(SpvReflectPrvParser *p_parser,
	...
	p_module->_internal->type_descriptions =
			(SpvReflectTypeDescription *)calloc(p_module->_internal->type_description_count,
	...
	for (size_t i = 0; i < p_parser->node_count; ++i) {
		...
		SpvReflectResult result = ParseType(p_parser, p_node, nullptr, p_module, p_type);
		if (result != SPV_REFLECT_RESULT_SUCCESS) {
			return result;
		}
		++type_index;
...
static SpvReflectResult ParseDescriptorBindings(SpvReflectPrvParser *p_parser,
	...
	for (size_t i = 0; i < p_parser->node_count; ++i) {
		...
		{
			const uint32_t resource_mask = SPV_REFLECT_TYPE_FLAG_EXTERNAL_SAMPLED_IMAGE
					| SPV_REFLECT_TYPE_FLAG_EXTERNAL_IMAGE;
	...
	if (p_module->descriptor_binding_count > 0) {
		qsort(p_module->descriptor_bindings, p_module->descriptor_binding_count,
				sizeof(*(p_module->descriptor_bindings)), SortCompareDescriptorBinding);
	}
...
static SpvReflectResult ParseDescriptorType(SpvReflectShaderModule *p_module) {
	...
	for (uint32_t descriptor_index = 0; descriptor_index < p_module->descriptor_binding_count;
		...
		case SPV_REFLECT_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER:
			p_descriptor->resource_type = (SpvReflectResourceType)(SPV_REFLECT_RESOURCE_FLAG_SAMPLER
...
static SpvReflectResult ParseUAVCounterBindings(SpvReflectShaderModule *p_module) {
	...
	for (uint32_t descriptor_index = 0; descriptor_index < p_module->descriptor_binding_count;
		...
		SpvReflectDescriptorBinding *p_counter_descriptor = nullptr;
		...
		if (p_counter_descriptor != nullptr) {
...
static SpvReflectResult ParseDescriptorBlockVariable(SpvReflectPrvParser *p_parser,
	...
	if (IsNotNull(p_type->members) && (p_type->member_count > 0)) {
		...
		p_var->members =
				(SpvReflectBlockVariable *)calloc(p_var->member_count, sizeof(*p_var->members));
		...
		for (uint32_t member_index = 0; member_index < p_type->member_count; ++member_index) {
			...
			if ((p_member_type->storage_class == SpvStorageClassPhysicalStorageBuffer)
				...
				if (p_member_type->op == SpvOpTypeArray
						|| p_member_type->op == SpvOpTypeRuntimeArray) {
				...
				if (!found_recursion) {
					...
					if (struct_type->struct_type_description) {
						...
						p_parser->physical_pointer_structs[p_parser->physical_pointer_struct_count]
								.struct_id = struct_id;
						p_parser->physical_pointer_structs[p_parser->physical_pointer_struct_count]
								.p_var = p_member_var;
						p_parser->physical_pointer_struct_count++;
			...
			bool is_struct = (p_member_type->type_flags & SPV_REFLECT_TYPE_FLAG_STRUCT)
					== SPV_REFLECT_TYPE_FLAG_STRUCT;
			if (is_struct) {
				if (!found_recursion) {
					SpvReflectResult result = ParseDescriptorBlockVariable(p_parser, p_module,
							p_member_type, p_member_var);
					if (result != SPV_REFLECT_RESULT_SUCCESS) {
						return result;
					}
				} else {
			...
			if (!has_non_writable
					&& (p_member_var->decoration_flags & SPV_REFLECT_DECORATION_NON_WRITABLE)) {
...
static SpvReflectResult ParseDescriptorBlockVariableSizes(SpvReflectPrvParser *p_parser,
		SpvReflectShaderModule *p_module, bool is_parent_root, bool is_parent_aos,
		bool is_parent_rta, SpvReflectBlockVariable *p_var) {
	...
	for (uint32_t member_index = 0; member_index < p_var->member_count; ++member_index) {
		...
		if (is_parent_root) {
			...
			p_member_var->absolute_offset = is_parent_aos
					? 0
	...
	for (uint32_t member_index = 0; member_index < p_var->member_count; ++member_index) {
		...
		case SpvOpTypeArray: {
			...
			if (is_struct) {
				if (p_member_var->flags & SPV_REFLECT_VARIABLE_FLAGS_PHYSICAL_POINTER_COPY) {
					...
					SpvReflectResult result = ParseDescriptorBlockVariableSizes(p_parser, p_module,
							false, true, is_parent_rta, p_member_var);
					if (result != SPV_REFLECT_RESULT_SUCCESS) {
						return result;
					}
				}
		...
		case SpvOpTypeStruct: {
			if (p_member_var->flags & SPV_REFLECT_VARIABLE_FLAGS_PHYSICAL_POINTER_COPY) {
				...
				SpvReflectResult result = ParseDescriptorBlockVariableSizes(p_parser, p_module,
						false, is_parent_aos, is_parent_rta, p_member_var);
				if (result != SPV_REFLECT_RESULT_SUCCESS) {
					return result;
				}
			}
	...
	p_last_member_var->padded_size =
			RoundUp(p_last_member_var->offset + p_last_member_var->size, SPIRV_DATA_ALIGNMENT)
			- p_last_member_var->offset;
...
static SpvReflectResult ParseDescriptorBlockVariableUsage(SpvReflectPrvParser *p_parser,
		SpvReflectShaderModule *p_module, SpvReflectPrvAccessChain *p_access_chain,
		uint32_t index_index, SpvOp override_op_type, SpvReflectBlockVariable *p_var) {
	// Clear the current variable's UNUSED flag
	p_var->flags &= ~SPV_REFLECT_VARIABLE_FLAGS_UNUSED;
	...
	case SpvOpTypeArray: {
		...
		if (index_index < p_access_chain->index_count) {
			...
			SpvReflectResult result = ParseDescriptorBlockVariableUsage(p_parser, p_module,
					p_access_chain, index_index, p_type->op, p_var);
			if (result != SPV_REFLECT_RESULT_SUCCESS) {
				return result;
			}
		} else {
	...
	case SpvOpTypeStruct: {
		...
		if (index_index < p_access_chain->index_count) {
			SpvReflectResult result = ParseDescriptorBlockVariableUsage(p_parser, p_module,
					p_access_chain, index_index, (SpvOp)INVALID_VALUE, p_member_var);
			if (result != SPV_REFLECT_RESULT_SUCCESS) {
				return result;
			}
...
static SpvReflectResult ParseDescriptorBlocks(SpvReflectPrvParser *p_parser,
	...
	for (uint32_t descriptor_index = 0; descriptor_index < p_module->descriptor_binding_count;
		...
		SpvReflectResult result =
				ParseDescriptorBlockVariable(p_parser, p_module, p_type, &p_descriptor->block);
		if (result != SPV_REFLECT_RESULT_SUCCESS) {
			return result;
		}

		for (uint32_t access_chain_index = 0; access_chain_index < p_parser->access_chain_count;
				++access_chain_index) {
			SpvReflectPrvAccessChain *p_access_chain =
					&(p_parser->access_chains[access_chain_index]);
			...
			result = ParseDescriptorBlockVariableUsage(p_parser, p_module, p_access_chain, 0,
					(SpvOp)INVALID_VALUE, &p_descriptor->block);
			if (result != SPV_REFLECT_RESULT_SUCCESS) {
				return result;
			}
		}
		...
		bool is_parent_rta =
				(p_descriptor->descriptor_type == SPV_REFLECT_DESCRIPTOR_TYPE_STORAGE_BUFFER);
		result = ParseDescriptorBlockVariableSizes(p_parser, p_module, true, false, is_parent_rta,
				&p_descriptor->block);
		if (result != SPV_REFLECT_RESULT_SUCCESS) {
			return result;
		}
...
static SpvReflectResult ParseFormat(const SpvReflectTypeDescription *p_type,
	...
	if (p_type->type_flags & SPV_REFLECT_TYPE_FLAG_VECTOR) {
		...
		switch (bit_width) {
		case 16: *p_format = SPV_REFLECT_FORMAT_R16_SFLOAT; break;
		case 32: *p_format = SPV_REFLECT_FORMAT_R32_SFLOAT; break;
		case 64: *p_format = SPV_REFLECT_FORMAT_R64_SFLOAT; break;
		}
		result = SPV_REFLECT_RESULT_SUCCESS;
	} else if (p_type->type_flags & (SPV_REFLECT_TYPE_FLAG_INT | SPV_REFLECT_TYPE_FLAG_BOOL)) {
		switch (bit_width) {
...
static SpvReflectResult ParseInterfaceVariable(SpvReflectPrvParser *p_parser,
		...
		const SpvReflectPrvDecorations *p_type_node_decorations, SpvReflectShaderModule *p_module,
		SpvReflectTypeDescription *p_type, SpvReflectInterfaceVariable *p_var,
		bool *p_has_built_in) {
	SpvReflectPrvNode *p_type_node = FindNode(p_parser, p_type->id);
	if (IsNull(p_type_node)) {
		return SPV_REFLECT_RESULT_ERROR_SPIRV_INVALID_ID_REFERENCE;
	...
	if (p_type->member_count > 0) {
		p_var->member_count = p_type->member_count;
		p_var->members =
				(SpvReflectInterfaceVariable *)calloc(p_var->member_count, sizeof(*p_var->members));
		if (IsNull(p_var->members)) {
			return SPV_REFLECT_RESULT_ERROR_ALLOC_FAILED;
		}
		...
		for (uint32_t member_index = 0; member_index < p_type_node->member_count; ++member_index) {
			...
			SpvReflectResult result = ParseInterfaceVariable(p_parser, nullptr,
					p_member_decorations, p_module, p_member_type, p_member_var, p_has_built_in);
			if (result != SPV_REFLECT_RESULT_SUCCESS) {
				SPV_REFLECT_ASSERT(false);
				return result;
			}
	...
	if (p_var_node_decorations != nullptr) {
...
static SpvReflectResult ParseInterfaceVariables(SpvReflectPrvParser *p_parser,
		SpvReflectShaderModule *p_module, SpvReflectEntryPoint *p_entry,
		uint32_t interface_variable_count, uint32_t *p_interface_variable_ids) {
	...
	for (size_t i = 0; i < interface_variable_count; ++i) {
		...
		SpvReflectResult result = ParseInterfaceVariable(p_parser, &p_node->decorations,
				&p_type_node->decorations, p_module, p_type, p_var, &has_built_in);
		if (result != SPV_REFLECT_RESULT_SUCCESS) {
			SPV_REFLECT_ASSERT(false);
			return result;
		}
...
static SpvReflectResult EnumerateAllPushConstants(SpvReflectShaderModule *p_module,
		size_t *p_push_constant_count, uint32_t **p_push_constants) {
...
static SpvReflectResult TraverseCallGraph(SpvReflectPrvParser *p_parser,
		SpvReflectPrvFunction *p_func, size_t *p_func_count, uint32_t *p_func_ids, uint32_t depth) {
	...
	for (size_t i = 0; i < p_func->callee_count; ++i) {
		SpvReflectResult result = TraverseCallGraph(p_parser, p_func->callee_ptrs[i], p_func_count,
				p_func_ids, depth + 1);
		if (result != SPV_REFLECT_RESULT_SUCCESS) {
			return result;
		}
...
static bool HasByteAddressBufferOffset(SpvReflectPrvNode *p_node,
	...
	return IsNotNull(p_node) && IsNotNull(p_binding)
			&& (p_node->op == SpvOpAccessChain || p_node->op == SpvOpInBoundsAccessChain)
			&& p_node->word_count == 6
...
static SpvReflectResult ParseByteAddressBuffer(SpvReflectPrvParser *p_parser,
		SpvReflectPrvNode *p_node, SpvReflectDescriptorBinding *p_binding) {
	...
	if (IsNull(p_next_node)) {
		...
		p_binding->byte_address_buffer_offsets[p_binding->byte_address_buffer_offset_count] =
				offset;
	...
	for (uint32_t i = 0; i < count; i++) {
		...
		case SpvOpShiftRightLogical: offset >>= value; break;
		case SpvOpIAdd: offset += value; break;
		case SpvOpISub: offset -= value; break;
		case SpvOpIMul: offset *= value; break;
		case SpvOpUDiv: offset /= value; break;
		case SpvOpSDiv:
...
static SpvReflectResult ParseFunctionParameterAccess(SpvReflectPrvParser *p_parser,
		uint32_t callee_function_id, uint32_t function_parameter_index, uint32_t *p_accessed) {
	SpvReflectPrvFunction *p_func = nullptr;
	for (size_t i = 0; i < p_parser->function_count; ++i) {
	...
	for (size_t i = 0; i < p_func->accessed_variable_count; ++i) {
		if (p_func->parameters[function_parameter_index]
				== p_func->accessed_variables[i].variable_ptr) {
			SpvReflectPrvAccessedVariable *p_var = &p_func->accessed_variables[i];
			if (p_var->function_id > 0) {
				SpvReflectResult result = ParseFunctionParameterAccess(p_parser, p_var->function_id,
						p_var->function_parameter_index, p_accessed);
				if (result != SPV_REFLECT_RESULT_SUCCESS) {
					return result;
				}
			} else {
...
static SpvReflectResult ParseStaticallyUsedResources(SpvReflectPrvParser *p_parser,
		SpvReflectShaderModule *p_module, SpvReflectEntryPoint *p_entry, size_t uniform_count,
	...
	SpvReflectResult result =
			TraverseCallGraph(p_parser, p_func, &called_function_count, nullptr, 0);
	if (result != SPV_REFLECT_RESULT_SUCCESS) {
		return result;
	}

	uint32_t *p_called_functions = nullptr;
	...
	if (called_function_count > 0) {
		qsort(p_called_functions, called_function_count, sizeof(*p_called_functions),
				SortCompareUint32);
	}
	...
	for (size_t i = 0, j = 0; i < called_function_count; ++i) {
		...
		while (p_parser->functions[j].id != p_called_functions[i]) { ++j; }
	...
	SpvReflectPrvAccessedVariable *p_used_accesses = (SpvReflectPrvAccessedVariable *)calloc(
	...
	result = IntersectSortedAccessedVariable(p_used_accesses, used_acessed_count, uniforms,
			uniform_count, &p_entry->used_uniforms, &used_uniform_count);
	if (result != SPV_REFLECT_RESULT_SUCCESS) {
	...
	result = IntersectSortedAccessedVariable(p_used_accesses, used_acessed_count, push_constants,
			push_constant_count, &p_entry->used_push_constants, &used_push_constant_count);
	if (result != SPV_REFLECT_RESULT_SUCCESS) {
		SafeFree(p_used_accesses);
		return result;
	}
	...
	for (uint32_t i = 0; i < p_module->descriptor_binding_count; ++i) {
		...
		for (uint32_t j = 0; j < used_acessed_count; j++) {
			...
			if (p_var->variable_ptr == p_binding->spirv_id) {
				if (p_var->function_id > 0) {
					result = ParseFunctionParameterAccess(p_parser, p_var->function_id,
							p_var->function_parameter_index, &p_binding->accessed);
					if (result != SPV_REFLECT_RESULT_SUCCESS) {
						SafeFree(p_used_accesses);
						return result;
					}
		...
		if (byte_address_buffer_offset_count > 0) {
			...
			if (multi_entrypoint) {
				...
				p_binding->byte_address_buffer_offset_count =
						(uint32_t)DedupSortedUint32(p_binding->byte_address_buffer_offsets,
								p_binding->byte_address_buffer_offset_count);
			}
		}
	}
...
static SpvReflectResult ParseEntryPoints(SpvReflectPrvParser *p_parser,
	...
	for (size_t i = 0; entry_point_index < p_parser->entry_point_count && i < p_parser->node_count;
			++i) {
		SpvReflectPrvNode *p_node = &(p_parser->nodes[i]);
		...
		result = ReadStr(p_parser, p_node->word_offset + name_start_word_offset, 0,
				p_node->word_count, &name_length_with_terminator, nullptr);
		if (result != SPV_REFLECT_RESULT_SUCCESS) {
			return result;
		}
		p_entry_point->name =
				(const char *)(p_parser->spirv_code + p_node->word_offset + name_start_word_offset);
		...
		uint32_t *p_interface_variables = nullptr;
		if (interface_variable_count > 0) {
			p_interface_variables =
					(uint32_t *)calloc(interface_variable_count, sizeof(*(p_interface_variables)));
		...
		result = ParseInterfaceVariables(p_parser, p_module, p_entry_point,
				interface_variable_count, p_interface_variables);
		if (result != SPV_REFLECT_RESULT_SUCCESS) {
			return result;
		}
		SafeFree(p_interface_variables);

		result = ParseStaticallyUsedResources(p_parser, p_module, p_entry_point, uniform_count,
				uniforms, push_constant_count, push_constants);
		if (result != SPV_REFLECT_RESULT_SUCCESS) {
			return result;
		}
	}
...
static SpvReflectResult ParseExecutionModes(SpvReflectPrvParser *p_parser,
	...
	if (IsNotNull(p_parser) && IsNotNull(p_parser->spirv_code) && IsNotNull(p_parser->nodes)) {
		for (size_t node_idx = 0; node_idx < p_parser->node_count; ++node_idx) {
			...
			// Find entry point
			SpvReflectEntryPoint *p_entry_point = nullptr;
			for (size_t entry_point_idx = 0; entry_point_idx < p_module->entry_point_count;
					++entry_point_idx) {
...
static SpvReflectResult ParsePushConstantBlocks(SpvReflectPrvParser *p_parser,
	...
	for (size_t i = 0; i < p_parser->node_count; ++i) {
		...
		p_parser->physical_pointer_count = 0;
		SpvReflectResult result =
				ParseDescriptorBlockVariable(p_parser, p_module, p_type, p_push_constant);
		if (result != SPV_REFLECT_RESULT_SUCCESS) {
			return result;
		}
		...
		for (uint32_t access_chain_index = 0; access_chain_index < p_parser->access_chain_count;
			...
			if (p_push_constant->spirv_id
					!= FindAccessChainBaseVariable(p_parser, p_access_chain)) {
				continue;
			}
			SpvReflectBlockVariable *p_var = (p_access_chain->base_id == p_push_constant->spirv_id)
					? p_push_constant
					: GetRefBlkVar(p_parser, p_access_chain);
			result = ParseDescriptorBlockVariableUsage(p_parser, p_module, p_access_chain, 0,
					(SpvOp)INVALID_VALUE, p_var);
			if (result != SPV_REFLECT_RESULT_SUCCESS) {
				return result;
			}
		}
		...
		result = ParseDescriptorBlockVariableSizes(p_parser, p_module, true, false, true,
				p_push_constant);
		if (result != SPV_REFLECT_RESULT_SUCCESS) {
			return result;
		}
...
static SpvReflectResult ParseSpecConstants(SpvReflectPrvParser *p_parser,
		SpvReflectShaderModule *p_module) {
	...
	for (uint32_t i = 0; i < p_parser->node_count; ++i) {
		...
		if (p_node->op == SpvOpDecorate) {
			...
			if (decoration == SpvDecorationSpecId) {
				SpvReflectSpecializationConstant *p_spec_constant =
						&(p_module->spec_constants[p_module->spec_constant_count]);
				...
				if (IsNotNull(target_node)) {
					...
					case SpvOpSpecConstantFalse:
						...
						break;
					case SpvOpSpecConstant:
						p_spec_constant->default_value_size =
								(target_node->word_count - 3) * sizeof(uint32_t);
						p_spec_constant->default_value =
								p_parser->spirv_code + target_node->word_offset + 3;
						break;
					}
					p_spec_constant->type_description =
							FindType(p_module, target_node->result_type_id);
				} else {
	...
	for (uint32_t i = 0; i < p_module->entry_point_count; ++i) {
		...
		for (uint32_t j = 0; j < p_module->descriptor_set_count; ++j) {
			...
			for (uint32_t k = 0; k < p_set->binding_count; ++k) {
				bool found = SearchSortedUint32(p_entry->used_uniforms, p_entry->used_uniform_count,
		...
		p_entry->descriptor_sets = nullptr;
		...
		for (uint32_t j = 0; j < p_module->descriptor_set_count; ++j) {
			...
			p_entry_set->bindings =
					(SpvReflectDescriptorBinding **)calloc(count, sizeof(*p_entry_set->bindings));
...
static SpvReflectResult ParseDescriptorSets(SpvReflectShaderModule *p_module) {
	...
	for (uint32_t i = 0; i < p_module->descriptor_binding_count; ++i) {
		...
		SpvReflectDescriptorSet *p_target_set = nullptr;
	...
	for (uint32_t i = 0; i < p_module->descriptor_set_count; ++i) {
		...
		p_set->bindings = (SpvReflectDescriptorBinding **)calloc(p_set->binding_count,
				sizeof(*(p_set->bindings)));
...
static SpvReflectResult CreateShaderModule(uint32_t flags, size_t size, const void *p_code,
		SpvReflectShaderModule *p_module) {
	...
	// Create parser
	SpvReflectResult result =
			CreateParser(p_module->_internal->spirv_size, p_module->_internal->spirv_code, &parser);

	// Generator
	{
		const uint32_t *p_ptr = (const uint32_t *)p_module->_internal->spirv_code;
		p_module->generator = (SpvReflectGenerator)((*(p_ptr + 2) & 0xFFFF'0000) >> 16);
	}
	...
	if (result == SPV_REFLECT_RESULT_SUCCESS) {
		...
		memset(p_module->descriptor_sets, 0,
				SPV_REFLECT_MAX_DESCRIPTOR_SETS * sizeof(*p_module->descriptor_sets));
...
SpvReflectResult spvReflectCreateShaderModule(size_t size, const void *p_code,
		SpvReflectShaderModule *p_module) {
	return CreateShaderModule(0, size, p_code, p_module);
}

SpvReflectResult spvReflectCreateShaderModule2(uint32_t flags, size_t size, const void *p_code,
		SpvReflectShaderModule *p_module) {
...
const SpvReflectEntryPoint *spvReflectGetEntryPoint(const SpvReflectShaderModule *p_module,
...
SpvReflectResult spvReflectEnumerateEntryPointDescriptorBindings(
		const SpvReflectShaderModule *p_module, const char *entry_point, uint32_t *p_count,
...
SpvReflectResult spvReflectEnumerateEntryPointDescriptorSets(const SpvReflectShaderModule *p_module,
		const char *entry_point, uint32_t *p_count, SpvReflectDescriptorSet **pp_sets) {
	if (IsNull(p_module)) {
		return SPV_REFLECT_RESULT_ERROR_NULL_POINTER;
	}
	if (IsNull(p_count)) {
...
SpvReflectResult spvReflectEnumerateInterfaceVariables(const SpvReflectShaderModule *p_module,
...
SpvReflectResult spvReflectEnumerateEntryPointInputVariables(const SpvReflectShaderModule *p_module,
		const char *entry_point, uint32_t *p_count, SpvReflectInterfaceVariable **pp_variables) {
	if (IsNull(p_module)) {
		return SPV_REFLECT_RESULT_ERROR_NULL_POINTER;
	}
	if (IsNull(p_count)) {
...
SpvReflectResult spvReflectEnumeratePushConstantBlocks(const SpvReflectShaderModule *p_module,
...
SpvReflectResult spvReflectEnumerateEntryPointPushConstantBlocks(
	...
	for (uint32_t i = 0; i < p_module->push_constant_block_count; ++i) {
		bool found = SearchSortedUint32(p_entry->used_push_constants,
				p_entry->used_push_constant_count, p_module->push_constant_blocks[i].spirv_id);
		if (found) {
...
SpvReflectResult spvReflectEnumerateSpecializationConstants(const SpvReflectShaderModule *p_module,
		uint32_t *p_count, SpvReflectSpecializationConstant **pp_constants) {
	if (IsNull(p_module)) {
		return SPV_REFLECT_RESULT_ERROR_NULL_POINTER;
	}
	if (IsNull(p_count)) {
...
const SpvReflectDescriptorBinding *spvReflectGetDescriptorBinding(
		const SpvReflectShaderModule *p_module, uint32_t binding_number, uint32_t set_number,
		SpvReflectResult *p_result) {
	const SpvReflectDescriptorBinding *p_descriptor = nullptr;
...
const SpvReflectDescriptorBinding *spvReflectGetEntryPointDescriptorBinding(
		const SpvReflectShaderModule *p_module, const char *entry_point, uint32_t binding_number,
...
const SpvReflectDescriptorSet *spvReflectGetDescriptorSet(const SpvReflectShaderModule *p_module,
		uint32_t set_number, SpvReflectResult *p_result) {
	const SpvReflectDescriptorSet *p_set = nullptr;
	if (IsNotNull(p_module)) {
...
const SpvReflectDescriptorSet *spvReflectGetEntryPointDescriptorSet(
		const SpvReflectShaderModule *p_module, const char *entry_point, uint32_t set_number,
		SpvReflectResult *p_result) {
	const SpvReflectDescriptorSet *p_set = nullptr;
	if (IsNotNull(p_module)) {
		const SpvReflectEntryPoint *p_entry = spvReflectGetEntryPoint(p_module, entry_point);
...
const SpvReflectInterfaceVariable *spvReflectGetInputVariableByLocation(
		const SpvReflectShaderModule *p_module, uint32_t location, SpvReflectResult *p_result) {
	...
	const SpvReflectInterfaceVariable *p_var = nullptr;
	if (IsNotNull(p_module)) {
...
const SpvReflectInterfaceVariable *spvReflectGetEntryPointInputVariableByLocation(
		const SpvReflectShaderModule *p_module, const char *entry_point, uint32_t location,
		SpvReflectResult *p_result) {
	if (location == INVALID_VALUE) {
		if (IsNotNull(p_result)) {
			*p_result = SPV_REFLECT_RESULT_ERROR_ELEMENT_NOT_FOUND;
...
const SpvReflectInterfaceVariable *spvReflectGetInputVariableBySemantic(
		const SpvReflectShaderModule *p_module, const char *semantic, SpvReflectResult *p_result) {
	if (IsNull(semantic)) {
		if (IsNotNull(p_result)) {
	...
	if (IsNotNull(p_module)) {
		for (uint32_t index = 0; index < p_module->input_variable_count; ++index) {
			const SpvReflectInterfaceVariable *p_potential = p_module->input_variables[index];
			if (p_potential->semantic != nullptr && strcmp(p_potential->semantic, semantic) == 0) {
				p_var = p_potential;
			}
		}
	}
...
const SpvReflectInterfaceVariable *spvReflectGetEntryPointInputVariableBySemantic(
		const SpvReflectShaderModule *p_module, const char *entry_point, const char *semantic,
		SpvReflectResult *p_result) {
	if (IsNull(semantic)) {
		if (IsNotNull(p_result)) {
			*p_result = SPV_REFLECT_RESULT_ERROR_NULL_POINTER;
...
const SpvReflectBlockVariable *spvReflectGetPushConstantBlock(
		const SpvReflectShaderModule *p_module, uint32_t index, SpvReflectResult *p_result) {
	const SpvReflectBlockVariable *p_push_constant = nullptr;
	if (IsNotNull(p_module)) {
...
SpvReflectResult spvReflectChangeDescriptorBindingNumbers(SpvReflectShaderModule *p_module,
	...
	SpvReflectDescriptorBinding *p_target_descriptor = nullptr;
	for (uint32_t index = 0; index < p_module->descriptor_binding_count; ++index) {
	...
	if (IsNotNull(p_target_descriptor)) {
		...
		if (new_binding_number != (uint32_t)SPV_REFLECT_BINDING_NUMBER_DONT_CHANGE) {
			uint32_t *p_code =
					p_module->_internal->spirv_code + p_target_descriptor->word_offset.binding;
		...
		if (new_set_binding != (uint32_t)SPV_REFLECT_SET_NUMBER_DONT_CHANGE) {
			uint32_t *p_code =
					p_module->_internal->spirv_code + p_target_descriptor->word_offset.set;
...
SpvReflectResult spvReflectChangeDescriptorBindingNumber(SpvReflectShaderModule *p_module,
		const SpvReflectDescriptorBinding *p_descriptor_binding, uint32_t new_binding_number,
		uint32_t optional_new_set_number) {
	return spvReflectChangeDescriptorBindingNumbers(p_module, p_descriptor_binding,
			new_binding_number, optional_new_set_number);
}

SpvReflectResult spvReflectChangeDescriptorSetNumber(SpvReflectShaderModule *p_module,
		const SpvReflectDescriptorSet *p_set, uint32_t new_set_number) {
	if (IsNull(p_module)) {
		return SPV_REFLECT_RESULT_ERROR_NULL_POINTER;
	}
	if (IsNull(p_set)) {
...
static SpvReflectResult ChangeVariableLocation(SpvReflectShaderModule *p_module,
		SpvReflectInterfaceVariable *p_variable, uint32_t new_location) {
